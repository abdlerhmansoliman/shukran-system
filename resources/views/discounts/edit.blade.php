    ...
    <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        <div class="mb-8 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                ...
                <h1 class="mt-2 text-3xl font-semibold tracking-tight text-slate-900">{{ __('Edit Discount') }}</h1>
                <p class="mt-2 text-sm text-slate-500">{{ __('Update discount template for :name.', ['name' => $discount->name]) }}</p>
            </div>
        ...
        <form method="POST" action="{{ route('discounts.update', $discount) }}" class="space-y-6">
            ...
            @include('discounts._form', [
                'discount' => $discount,
                'submitLabel' => __('Update Discount'),
